...
// 👇 Ajusta el puerto si tu CartService usa otro
const CART_API_BASE =
...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadCart = async () => {
      const isLocalUser = String(user?.id || '').startsWith('local-');
      if (!user?.id || isLocalUser) return;
      ...
      try {
        ...
        console.warn("No se pudo cargar el carrito del backend:", err?.message || err);
      }
    };
  ...
  const addToCart = useCallback(
    async (product, qty = 1) => {
      ...
      const isLocalUser = String(user?.id || '').startsWith('local-');
      if (!user?.id || isLocalUser) {
        setCartItems((prev) => {
          const idx = prev.findIndex((it) => it.id === pid);
          if (idx >= 0) {
            const copy = [...prev];
            const current = copy[idx];
            copy[idx] = { ...current, qty: (current.qty || 1) + qty };
            return copy;
          }
